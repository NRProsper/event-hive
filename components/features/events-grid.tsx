...
import GridLoadingSkeleton from "@/components/grid-loading";

export default  function EventsGrid() {
    const {data, isLoading, isPending} = useEvents();
    ...
    return (
        (isLoading || allEvents === undefined || isPending) ?
            <GridLoadingSkeleton/> :
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-x-6 gap-y-[30px]">
                {
                    allEvents?.map((event, idx) => (
                        <div key={idx}
                             className="group rounded-xl shadow-xl hover:shadow-2xl p-6 border border-gray-300">
